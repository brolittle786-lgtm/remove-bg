...
export default function ResultPreview({ originalUrl, finalUrl, onBackToEdit, onReset }) {
  ...
  return (
    <section className="fade-up" style={{ padding: '0 28px 72px', maxWidth: 1040, margin: '0 auto' }}>
      {/* Success bar */}
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <p style={{ fontWeight: 700, color: '#15803D', fontSize: 15, fontFamily: 'var(--font-d)' }}>
            Your image is ready to download!
          </p>
          <p style={{ color: '#166534', fontSize: 13, marginTop: 2 }}>
            Background removed and edits applied successfully.
          </p>
        </div>
        {downloaded && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 20, alignItems: 'start' }}>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <div style={{
              display: 'flex', background: 'var(--surface-2)', borderRadius: 9,
              padding: 3, gap: 2,
            }}>
              {[
            ...
            <span style={{ fontSize: 12, color: 'var(--ink-3)', fontWeight: 500 }}>
              {view === 'before' ? 'Showing original upload' : 'Showing processed result'}
            </span>
          </div>

          {/* Image display */}
          <div
            className={view === 'after' ? 'checker' : ''}
            style={{
              minHeight: 380, display: 'flex', alignItems: 'center', justifyContent: 'center',
              padding: 36, transition: 'all 0.25s',
              background: view === 'before' ? 'var(--surface-2)' : undefined,
            }}
          >
            <img
              key={view}
              src={view === 'before' ? originalUrl : finalUrl}
              alt={view}
              style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="card" style={{ padding: 20 }}>
            <p style={{ fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--ink-3)', marginBottom: 16 }}>
              Export
            </p>
            ...
            <div style={{
              ...
              {[
                ...
                ['Size', 'Ready for any design tool'],
              ].map(([k, v]) => (
          ...
          <div className="card" style={{ padding: 20 }}>
            <p style={{ fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--ink-3)', marginBottom: 12 }}>
              Edit More
            </p>
      ...
      <style>{`
        @media (max-width: 680px) {
          section[style*="1fr 260px"] { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </section>
  )
